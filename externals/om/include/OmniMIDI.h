...
// Send short messages through KDMAPI. (Like midiOutShortMsg)
extern "C" VOID KDMAPI(SendDirectData)(DWORD dwMsg);

// Send short messages through KDMAPI like SendDirectData, but bypasses the buffer. (Like midiOutShortMsg)
extern "C" VOID KDMAPI(SendDirectDataNoBuf)(DWORD dwMsg);

// Send long messages through KDMAPI. (Like midiOutLongMsg)
extern "C" UINT KDMAPI(SendDirectLongData)(MIDIHDR* IIMidiHdr, UINT IIMidiHdrSize);

// Send long messages through KDMAPI like SendDirectLongData, but bypasses the buffer. (Like midiOutLongMsg)
extern "C" UINT KDMAPI(SendDirectLongDataNoBuf)(LPSTR MidiHdrData, DWORD MidiHdrDataLen);

// Prepares the long data, and locks its memory to prevent apps from writing to it.
extern "C" UINT KDMAPI(PrepareLongData)(MIDIHDR* IIMidiHdr, UINT IIMidiHdrSize);

// Unlocks the memory, and unprepares the long data.
extern "C" UINT KDMAPI(UnprepareLongData)(MIDIHDR* IIMidiHdr, UINT IIMidiHdrSize);
...
extern "C" VOID KDMAPI(LoadCustomSoundFontsList)(LPWSTR Directory);

// timeGetTime, but 64-bit
extern "C" DWORD64 KDMAPI(timeGetTime64)();

// modMessage
extern "C" MMRESULT KDMAPI(modMessage)(UINT uDeviceID, UINT uMsg, DWORD_PTR dwUser, DWORD_PTR dwParam1, DWORD_PTR dwParam2);
#endif